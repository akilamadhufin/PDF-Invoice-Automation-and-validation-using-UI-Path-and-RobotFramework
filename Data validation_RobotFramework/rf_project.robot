...
*** Variables ***
# Global variables
${PATH}    D:\\HAMK\\2nd Year\\2nd Semester\\Module 1\\Software Robotics and Automation\\Group Projects\\RobotFramework\\
@{ListToDB}
${InvoiceNumber}    empty
...
Ensure Invoice Status Exists
    ...
    IF    ${count} == 0
        ...
        Execute Sql String    INSERT INTO invoicestatus (id, name) VALUES (0, 'Approved'), (1, 'Reference Error'), (2, 'IBAN Error'), (3, 'Amount Mismatch'),(4, 'Date Mismatch');
    ELSE
...
Add Invoice Header To DB
    ...
    ${checkDate}=    Subtract Date From Date    ${innvoiceDate}    ${dueDate}
    ...
    ${refResult}=    isRefCorrect    ${items}[2]
    ...
    IF    ${checkDate}> 0
        ...
        ${commentOfInvoice}=    Set Variable    Date Mismatch

    END


    ${insertStmt}=    Set Variable    insert into dt_invoiceheader (invoicenumber, companyname, companycode, referencenumber, invoicedate, duedate, bankaccountnumber, amountexclvat, vat, totalamount, invoicestatus_id, comment) values ('${items}[0]', '${items}[1]', '${items}[5]', '${items}[2]', '${innvoiceDate}', '${dueDate}', '${items}[6]', '${items}[7]', '${items}[8]', '${items}[9]', '${statusOfInvoice}', '${commentOfInvoice}');
    #Log    ${insertStmt}
...
Add Invoice Row To DB
    ...
    [Arguments]    ${items}
    Make Connection    ${dbname}
    ${insertStmt}=    Set Variable    insert into dt_invoicerow (invoicenumber, rownumber, description, quantity, unit, unitprice, vatpercent, vat, total) values ('${items}[0]', '${items}[1]', '${items}[2]', '${items}[3]', '${items}[4]', '${items}[5]', '${items}[6]', '${items}[7]', '${items}[8]');
    Execute Sql String    ${insertStmt}
            ...
            ELSE
                ...
                FOR    ${headerElement}    IN    @{headers}
                    ...
                    IF    '${headerItems}[0]' == '${InvoiceNumber}'
                        ...
                        ${refResult}=    isRefCorrect    ${headerItems}[2]
                        IF    not ${refResult}
                            ${statusOfInvoice}=    Set Variable    1
                            ${commentOfInvoice}=    Set Variable    Reference number error
                        END
